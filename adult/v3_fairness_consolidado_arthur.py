# -*- coding: utf-8 -*-
"""v3-fairness-consolidado-arthur.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1yo-w8w2ztFN0EGdpVNP8CCA6QDLCeoMU
...
#removendo a coluna fnlwgt (sugestão dos scikit learn)
adult_df = adult_df.drop("fnlwgt", axis=1)
#adult_df = data.select_dtypes(exclude='object')
adult_df
...
def encodeR(train, test):
    ...
    pd_data_train = pd.DataFrame(encoded_train,
                                 columns=[
                                     'Workclass', 'Education', 'MartialStatus',
    ...
    pd_data_test = pd.DataFrame(encoded_test,
                                columns=[
                                    'Workclass', 'Education', 'MartialStatus',
                                    'Occupation', 'Relationship', 'Race',
                                    'Sex', 'Country', 'Target'
...
def calcProportionPositive(df, attribute, attribute_value, class_label,
                           class_value):
    details = df.apply(lambda x: True
                       if x[attribute] == attribute_value else False,
                       axis=1)  #homens
    ...
    group_positive = group.apply(lambda x: True
                                 if x[class_label] == class_value else False,
                                 axis=1)  #>50k
...
def attributeAccuracy(df, pred_column, true_column):
    dfDetailsPred = df.apply(lambda x: True
                             if x[pred_column] == x[true_column] else False,
                             axis=1)
...
def truePositive(df, pred_column, true_column):
    dfDetailsPred = df.apply(
        lambda x: True
        if x[pred_column] == x[true_column] and x[pred_column] == 1 else False,
        axis=1)
    num_rowsPred = len(dfDetailsPred[dfDetailsPred == True].index)
...
def trueNegative(df, pred_column, true_column):
    dfDetailsPred = df.apply(
        lambda x: True
        if x[pred_column] == x[true_column] and x[pred_column] == 0 else False,
        axis=1)
    num_rowsPred = len(dfDetailsPred[dfDetailsPred == True].index)
...
def accuracyByGroup(X_t, y_test, y_pred, sensitive_attribute, group_b,
                    group_w):
    ...
    dfAttSens = dfAttSens.drop(columns=[
        'Age', 'Education-Num', 'CapitalGain', 'CapitalLoss', 'Hoursperweek',
        'Workclass_encoded', 'Education_encoded', 'MartialStatus_encoded',
...
clf_list = [(lr, "Logistic"), (gnb, "Naive Bayes"),
...
def criaCandidatos(df_x, df_y, att_sens, att_value, target_class, prob_full):
    ...
    candidates = df_x.loc[(df_x[att_sens] == att_value)
                          & (df_x['true_class'] == target_class)]
...
def massageChanges(df, sensitive_att, b_code, w_code, class_label,
                   target_class):

    women, women_positive_rate = calcProportionPositive(
        df, sensitive_att, b_code, class_label, 1)
    men, men_positive_rate = calcProportionPositive(df, sensitive_att, w_code,
                                                    class_label, 1)
...
def attributeCount(df, att_column, att_value):
    dfDetailsPred = df.apply(lambda x: True
                             if x[att_column] == att_value else False,
                             axis=1)
...
def attributeTrue(df, att_column, att_value):
    dfDetailsPred = df.apply(
        lambda x: True
        if x[att_column] == att_value and x['Target_encoded'] == 1 else False,
        axis=1)
    num_rowsPred = len(dfDetailsPred[dfDetailsPred == True].index)
...
def attributeFalse(df, att_column, att_value):
    dfDetailsPred = df.apply(
        lambda x: True
        if x[att_column] == att_value and x['Target_encoded'] == 0 else False,
        axis=1)
    num_rowsPred = len(dfDetailsPred[dfDetailsPred == True].index)
...
def findFav(ttdict, pa, label):
    ...
    pos_rates = [
        train[(train[pa] == u) & (train[label] == 1)].shape[0] /
        train[train[pa] == u].shape[0] for u in train[pa].unique()
    ]
...
def fairCorrectPSP(df, pa, fav, d=0):
    ...
    fav_pos = (df2[(df2[pa] == fav)
                   & (df2["Target_encoded"] == 1)].sort_values('pPos',
                                                               ascending=True))
    #print(fav_pos)
    fav_neg = (df2[(df2[pa] == fav)
                   & (df2["Target_encoded"] == 0)].sort_values(
                       'pPos', ascending=False))
    #print(fav_neg)
    unfav_pos = (df2[(df2[pa] != fav)
                     & (df2["Target_encoded"] == 1)].sort_values(
                         'pPos', ascending=True))
    #print(unfav_pos)
    unfav_neg = (df2[(df2[pa] != fav)
                     & (df2["Target_encoded"] == 0)].sort_values(
                         'pPos', ascending=False))
    #print(unfav_neg)
    ...
    if d < 1:
        ...
        # elements to add
        ext_fn = pd.concat([fav_neg] * math.ceil(fav_move / fav_neg.shape[0]),
                           ignore_index=True)
        ext_up = pd.concat([unfav_pos] *
                           math.ceil(unfav_move / unfav_pos.shape[0]),
                           ignore_index=True)
        ...
        # add to fn and up
        fav_neg = pd.concat([fav_neg, ext_fn.head(fav_move)],
                            ignore_index=True)
        unfav_pos = pd.concat([unfav_pos, ext_up.head(unfav_move)],
                              ignore_index=True)
...
def discriminationClassifier(df, predictions, att_sensitive, sensitive_label):
    ...
    details = df.apply(lambda x: True
                       if x[att_sensitive] == sensitive_label else False,
                       axis=1)
...
def criaModelos(df_list, X_test_target,  y_test, rstate):
  ...
  return result_scores, group_metrics, disc_metrics
"""# Criando os folds"""


def reportFairness(dfs, X_test, y_test, state, i_tex):
    scores, groups, discs = criaModelos(dfs, X_test, y_test, state)
    ...
    for score in scores:
        ...
        score.to_latex(latexName,
                       caption=score.name,
                       label=score.name + " " + str(i_tex))
        i_tex = i_tex + 1
    ...
    for group in groups:
        ...
        group.to_latex(latexName,
                       caption=group.name,
                       label=group.name + " " + str(i_tex))
        i_tex = i_tex + 1
    ...
    for key in discs.keys():
        ...
        mini.to_latex(latexName,
                      caption=mini.name,
                      label=mini.name + " " + str(i_tex))
        i_tex = i_tex + 1

    return scores, groups, discs


def aplicaMassageamento(df):
    #print("\nMassageamento\n")
    promo, demo = ranker(df, 'Sex_encoded', 1)
    promo = promo.sort_values(by=['predict_proba'],
    ...
    m, discriminationDataset = massageChanges(df, 'Sex_encoded', 0, 1,
                                              'Target_encoded', 1)
    #print(discriminationDataset)
    #print(
    #    "Quantidade de trocas a serem realizadas pela técnica de Massageamento: {}"
    #    .format(ceil(m)))
    ...
    m, discriminationDataset = massageChanges(adult_df_numerical_massage,
                                              'Sex_encoded', 0, 1,
                                              'Target_encoded', 1)
    #print(discriminationDataset)
    #print(
...
def aplicaAmostraPref(df):
    ...
    menPSP, men_positive_rate_PSP = calcProportionPositive(
        dfPSP, 'Sex_encoded', 1, 'Target_encoded', 1)

    womenPSP, women_positive_rate_PSP = calcProportionPositive(
        dfPSP, 'Sex_encoded', 0, 'Target_encoded', 1)
...
def calcMediaDesvClassificador(full_report, classificador, conjunto):
    ...
    classificador_full_report_bruto_folds['Conjunto de dados'] = "Bruto"
...
skf = StratifiedKFold(2, random_state=42, shuffle=True)
skf.get_n_splits(x, y)

scores_full_report = pd.DataFrame(data=None)  #inserir linha dummy (0,0,0,0,0)
groups_full_report = pd.DataFrame(data=None)

for train_index, test_index in skf.split(x, y):
    X_train, X_test = x.loc[x.index.intersection(train_index)], x.loc[
        x.index.intersection(test_index)]
    y_train, y_test = y.loc[y.index.intersection(train_index)], y.loc[
        y.index.intersection(test_index)]

    X_train['Target'] = y_train  #>50k ou #<=50k
    X_test['Target'] = y_test

    #display(X_train)
    #display(X_test)

    encoded_x_train, encoded_x_test, encoded_y_train, encoded_y_test = encodeR(
        X_train, X_test)
    #display(encoded_x_train)
    #display(encoded_x_test)

    #Quantos homens estão no fold, e desses quantos tem renda >50k?
    men, men_positive_rate = calcProportionPositive(encoded_x_train,
                                                    'Sex_encoded', 1,
                                                    'Target_encoded', 1)

    #Quantas mulheres existem no fold antes da redução de features, e quantas delas tem renda >50k?
    women, women_positive_rate = calcProportionPositive(
        encoded_x_train, 'Sex_encoded', 0, 'Target_encoded', 1)

    #print(
    #    "Atributo = gender\tGrupo favorecido = men\tGrupo desfavorecido = women\n\n"
    #)
    ...
    encoded_x_train_massageado = aplicaMassageamento(
        encoded_x_train
    )  #retornar o y_train do massageamento e da amostragem preferencial
    encoded_x_train_massageado.name = "Aplicado massageamento"
    encoded_x_train_amostraPref = aplicaAmostraPref(
        encoded_x_train
    )  #retornar o y_train do massageamento e da amostragem preferencial
    ...
    dfs = [
        encoded_x_train, encoded_x_train_massageado,
        encoded_x_train_amostraPref
    ]

    scores, groups, discs = reportFairness(dfs, encoded_x_test, encoded_y_test,
                                           42, i_tex)
